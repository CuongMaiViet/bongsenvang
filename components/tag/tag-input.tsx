...
export interface TagInputProps
  ...
  value?: string | number | readonly string[] | { id: string; value: string }[];
...
const TagInput = React.forwardRef<HTMLInputElement, TagInputProps>(
  (props, ref) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (
        ...
        if (
          ...
          !autocompleteOptions?.some((option) => option.value === newTagText)
        ) {
          toast({
            title: "Invalid Tag",
        ...
        if (
          newTagText &&
          (allowDuplicates || !tags.some((tag) => tag.value === newTagText)) &&
          (maxTags === undefined || tags.length < maxTags)
        ) {
          setTags([...tags, { id: newTagId, value: newTagText }]);
          onTagAdd?.(newTagText);
    ...
    const removeTag = (idToRemove: string) => {
      ...
      onTagRemove?.(tags.find((tag) => tag.id === idToRemove)?.value || "");
      setTagCount((prevTagCount) => prevTagCount - 1);
    };
    ...
    const truncatedTags = truncate
      ? tags.map((tag) => ({
          id: tag.id,
          value:
            tag.value?.length > truncate
              ? `${tag.value.substring(0, truncate)}...`
              : tag.value,
        }))
      : displayedTags;
